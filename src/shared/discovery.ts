...
import { xorDecryptInPlace, xorEncryptInPlace } from './cipher';

export const KASA_DISCOVERY_PORT = 9999;
const KASA_DISCOVERY_PROBE = '{"system":{"get_sysinfo":{}}}';

// Encrypted probe payload built once at module load and reused across every discovery
// sweep. dgram.send only reads from the buffer (never mutates), so sharing is safe.
const KASA_DISCOVERY_PROBE_BUFFER = xorEncryptInPlace(Buffer.from(KASA_DISCOVERY_PROBE, 'utf8'));
...
// All host IPs in each local /24 we're attached to (excluding our own). Used for paced UDP
// unicast sweeps when broadcast misses devices. Networks that aren't exactly /24 are skipped
// — sweeping a /23 or larger would flood, and treating a /25 or smaller as /24 would send
// probes outside the actual subnet (wasted traffic + neighbor confusion).
function* localSubnetIps(): Generator<string> {
    ...
    for (const name of Object.keys(ifaces)) {
        for (const info of ifaces[name] || []) {
            if (info.family !== 'IPv4' || info.internal) continue;
            const mask = info.netmask.split('.').map(Number);
            // Require exact /24 (255.255.255.0). The mask[3] === 0 check rejects /25 etc.
            if (mask.length !== 4 || mask[0] !== 255 || mask[1] !== 255 || mask[2] !== 255 || mask[3] !== 0) continue;
            const ip = info.address.split('.').map(Number);
...
export async function discoverKasa(durationMs: number = 2500, console?: Console): Promise<KasaDiscoveredDevice[]> {
    ...
    const probe = KASA_DISCOVERY_PROBE_BUFFER;
    ...
    return new Promise((resolve, reject) => {
        ...
        socket.bind(0, async () => {
            ...
            // socket.send throws synchronously (ERR_SOCKET_DGRAM_NOT_RUNNING) if the
            // deadline timer fires and closes the socket mid-sweep. Wrap both loops so
            // that race can't crash the discovery promise — we just stop sending.
            try {
                for (const addr of broadcastAddresses()) socket.send(probe, KASA_DISCOVERY_PORT, addr);
            } catch {
                return;
            }
            ...
            for (const addr of localSubnetIps()) {
                ...
                try {
                    ...
                    // Socket was closed (deadline elapsed) — stop sending.
                    break;
                }
