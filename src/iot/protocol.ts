import net from 'net';
import { xorDecrypt, xorEncryptInPlace } from '../shared/cipher';
...
export async function kasaIotCall(options: KasaIotOptions, command: Record<string, any>): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
        ...
        const consume = (n: number): Buffer => {
            ...
            queuedBytes -= n;
            if (chunks[0].length === n) return chunks.shift()!;
        ...
        socket.on('close', () => {
            if (!settled) finish(new Error('kasa iot socket closed before response'));
        });
